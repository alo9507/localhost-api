...
describe('Integration Test queries', () => {
  ...
  test('should fetch only viable users according to user visibility and ShowMeCriteria of both parties', async () => {
    ...
    const updateShowMeCriteriaResult = await apolloFetch({
      query: UPDATE_SHOWME_CRITERIA,
      variables: { input: updateShowMeCriteriaInput }
    });
    ...
    const updateShowMeCriteriaResult_2 = await apolloFetch({
      query: UPDATE_SHOWME_CRITERIA,
      variables: { input: updateShowMeCriteriaInput_2 }
    });
    ...
    const updateShowMeCriteriaResult_3 = await apolloFetch({
      query: UPDATE_SHOWME_CRITERIA,
      variables: { input: updateShowMeCriteriaInput_3 }
    });
    ...
    expect(new Set(viableUsersResult2.data.getViableUsers)).toEqual(
      new Set([{ id: 'male_25_visible' }, { id: 'female_25_visible' }])
    );
  });
});
