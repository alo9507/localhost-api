  ...
  filters.map(([filterKey, filterValue], currentFilterEntry) => {
    ...
    switch (filterOperator) {
      ...
      case 'gt':
        ...
        break;
      case 'between':
        conditionalString = conditionalString.concat(" ", `n.${filterKey} > ${wrapInString(filterTarget[0])} AND n.${filterKey} < ${wrapInString(filterTarget[1])}`);
        break;
    ...
    if (currentFilterEntry < filtersLength - 1) { conditionalString = conditionalString.concat(" ", "AND"); }
  });
